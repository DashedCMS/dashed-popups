<popup class="space-y-8 p-4 bg-primary-800 text-white rounded-lg" wire:submit="submit">
            ...
            <div class="flex">
                ...
                <div class="ml-3">
                    <p class="text-sm font-bold text-green-800">
                        {{ Translation::get('popup-sent', 'popups', 'Het popupulier is verzonden') }}
                    </p>
                </div>
            </div>
        </div>
            ...
            @if($field->stack_start)
                <div class="grid gap-2">
                    ...
                    <x-dynamic-component wire:key="field.{{ $loop->index }}"
                                         :component="'popup-components.' . $field->type" :field="$field" :loop="$loop"
                                         :values="$values"
                                         :fields="$this->popupFields"/>
                    @if($field->stack_end)
                </div>
            @endif
        @endforeach
    ...
    @if($buttonTitle)
        <div class="mt-auto">
            ...
            <button type="submit"
                ...
                <span>
                    {{ $buttonTitle ?: Translation::get('submit', str($popup->name)->slug() . '-popup', 'Verstuur') }}
                </span>
            </button>
            @if($buttonTitle)
        </div>
